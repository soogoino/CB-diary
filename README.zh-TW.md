    ...
    **[English](README.md) | 繁體中文**
    ...
    [![網站](https://img.shields.io/badge/官方網站-CB%20Diary-7c3aed?style=for-the-badge&logo=github-pages&logoColor=white)](https://soogoino.github.io/CB-diary/)
    ...
    - **自訂卡片背景** — 匯入任意 PNG/JPG 圖片作為卡片背景（自動縮放至 1080×1920）；可管理多張自訂卡片並各自命名
    - **卡片管理** — 長壓自訂卡片 Chip 可透過彈出視窗重新命名或刪除（含二次確認）
    - **文字色彩** — 每張自訂卡片獨立設定淺色 / 深色文字，支援即時預覽
    - **文字底圖** — 可選在圖片背景的文字後方加上半透明底圖，增強可讀性
    - **分享 / 儲存** — 直接分享到其他 App 或儲存至裝置
    - **卡片照片** — 可選擇將當日打卡照片嵌入卡片
    - **暱稱與統計標題** — 暱稱（若設定）置中顯示於卡片最頂端；「CB diary」與日期同行左右分佈
    ...
    #### 摘要卡片
    - [x] `CardData` — 彙整當日快照、7 日平均、連續天數、輪替問答、暱稱、累計天數
    - [x] `SummaryCardContent` — 1080×1920 全解析度 Composable（density=1 離屏渲染）
    - [x] 多主題（Midnight / Crimson / Ocean / Forest / Minimal 等），全部免費開放
    - [x] **自訂卡片背景匯入** — 單張圖片（PNG/JPG，自動縮放）透過 `TemplateImporter` 處理
    - [x] **自訂卡片管理** — 命名、重新命名、刪除（長壓彈出對話框）；名稱持久化至 `name.txt`
    - [x] **文字色彩方案** — 每張卡片獨立淺色 / 深色切換，即時預覽
    - [x] **文字底圖** — 文字後方半透明底圖（DataStore 設定持久化）
    - [x] 卡片照片開關（可選擇嵌入當日打卡照片）
    - [x] 暱稱置中顯示於卡片頂端；累計天數顯示於最佳連續天數旁
    - [x] 分享 / 儲存 PNG

    ### 待實作 📋

    - [ ] 單元測試 / UI 測試
    - [ ] 自訂卡片版面格式（spec JSON + zip 匯入）
    - [ ] 主螢幕 Widget
    - [ ] 鎖盒整合
    - [ ] 綁定、社群功能
