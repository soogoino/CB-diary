            ...
            DiaryTheme(darkTheme = darkTheme) {
                Surface(
                    ...
                    if (!data.onboardingCompleted) {
                        ...
                        OnboardingScreen(
                            onComplete = {
                                // completeOnboarding() writes encryptedPrefs synchronously before
                                // calling this lambda, so we can safely read _lockEnabled here.
                                val lockEnabled = encryptedPrefs.getBoolean(Constants.KEY_LOCK_ENABLED, false)
                                _lockEnabled.value = lockEnabled
