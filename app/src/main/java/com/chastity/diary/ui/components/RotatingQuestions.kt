    ...
    QuestionSection(
        ...
        if (entry.exercised) {
            ...
            MultiSelectChipGroup(
                options = stringArrayResource(R.array.exercise_types_array).toList(),
                selectedOptions = entry.exerciseTypes,
                storageKeys = Constants.EXERCISE_TYPES,
...
private fun ExposedLockQuestion(
    ...
    QuestionSection(
        ...
        if (entry.exposedLock) {
            ...
            Spacer(modifier = Modifier.height(8.dp))
            MultiSelectChipGroup(
                options = stringArrayResource(R.array.exposed_locations_array).toList(),
                selectedOptions = entry.exposedLocations,
                storageKeys = Constants.EXPOSED_LOCATIONS,
...
private fun KeyholderInteractionQuestion(
    ...
    QuestionSection(
        ...
        if (entry.keyholderInteraction) {
            ...
            Spacer(modifier = Modifier.height(8.dp))
            MultiSelectChipGroup(
                options = stringArrayResource(R.array.interaction_types_array).toList(),
                selectedOptions = entry.interactionTypes,
                storageKeys = Constants.INTERACTION_TYPES,
                onSelectionChange = { types ->
    ...
    QuestionSection(
        ...
        FlowRow(
            ...
            displayLabels.forEachIndexed { index, label ->
                val key = keyList.getOrElse(index) { label }
                FilterChip(
                    selected = entry.cleaningType == key,
                    onClick = { onEntryUpdate(entry.copy(cleaningType = key)) },
                    label = { Text(label) },
...
private fun SocialActivitiesQuestion(
    ...
    QuestionSection(
        ...
        Spacer(modifier = Modifier.height(8.dp))
        MultiSelectChipGroup(
            options = stringArrayResource(R.array.social_activities_array).toList(),
            selectedOptions = entry.socialActivities,
            storageKeys = Constants.SOCIAL_ACTIVITIES,
